...
async def advantage_spoll_choker(bot, query):
    ...
    if files:
        ...
        k = await query.message.edit(f'⚠️ Hey, {query.from_user.first_name}! YOUR REQUESTED MOVIE IS NOT IN MY DATABASE, ASK THE BOSS😣 ⚠️')
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer(f"Hey, {query.from_user.first_name}! JOIN THE CHANNEL FIRST AND THEN CLICK THE BUTTON😣",show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        cap = f"<b>🎬 Title:</b> {search}\n</b>\n<b><a href='https://t.me/vibeflix_nums'>◆NEW RELEASE◆</a></b>\n\n<b>✍️ Note:</b> <s>🔘THIS MESSAGE WILL DELETE IN 10 MINUTES🔘</s>"
